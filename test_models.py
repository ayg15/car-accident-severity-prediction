...
def main():
    ...
    # Logistic Regression
    logreg_model, y_train_pred_lr, y_test_pred_lr, scaler, logmetrics = (
        train_logistic_regression(X_train, y_train, X_test, y_test)
    )
    # print_metrics(y_test, y_test_pred_lr, 'Logistic Regression')

    # Decision Tree
    dt_model, y_train_pred_dt, y_test_pred_dt, dtmetrics = train_decision_tree(
        X_train,
        y_train,
        X_test,
        y_test,
    ...
    # Random Forest
    rf_model, y_train_pred_rf, y_test_pred_rf, rfmetrics = train_random_forest(
        X_train,
        y_train,
        X_test,
        y_test,
    ...
    bag_model, y_train_pred_bag, y_test_pred_bag, bagmetrics = (
        train_bagging_decision_tree(
            X_train,
            y_train,
            X_test,
    ...
    results = [
        ...
        (
            ...
            dt_model,
            precision_score(
                y_test, y_test_pred_dt, average="weighted", zero_division=0
            ),
        ),
        (
            "Tuned Decision Tree",
            best_dt,
            precision_score(
                y_test, y_test_pred_bestdt, average="weighted", zero_division=0
            ),
        ),
        (
            "Random Forest",
        ...
        (
            ...
            bag_model,
            precision_score(
                y_test, y_test_pred_bag, average="weighted", zero_division=0
            ),
        ),
    ]
